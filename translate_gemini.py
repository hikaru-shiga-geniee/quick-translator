...
- Output ONLY the translation, no explanations"""
        ...
        try:
            ...
            try:
                ...
                # ヘッダー方式を優先的に使用
                result = subprocess.run(
                    curl_cmd_header, capture_output=True, text=True, timeout=30
                )
                ...
                if result.returncode != 0:
                    ...
                    result = subprocess.run(
                        curl_cmd_url_param, capture_output=True, text=True, timeout=30
                    )
            ...
            # 翻訳結果の抽出
            if (
                "candidates" not in response_json
                or not response_json["candidates"]
                or "content" not in response_json["candidates"][0]
                or "parts" not in response_json["candidates"][0]["content"]
                or not response_json["candidates"][0]["content"]["parts"]
            ):
                raise ValueError("エラー: 翻訳結果が取得できませんでした")

            translated_text = response_json["candidates"][0]["content"]["parts"][0][
                "text"
            ]
