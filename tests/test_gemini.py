...
class TestQuickTranslatorGemini:
    ...
    def translator(self):
        ...
        with patch.object(QuickTranslator, '_get_api_key', return_value='test-gemini-api-key'):
            return QuickTranslator()

    def test_init(self, mocker):
        """Test QuickTranslator initialization."""
        mock_get_api_key = mocker.patch.object(QuickTranslator, '_get_api_key', return_value='test-gemini-api-key')
    ...
    def test_translate_with_api_no_key(self, mocker):
        ...
        with patch.object(QuickTranslator, '_get_api_key', return_value=None):
            ...
            with pytest.raises(ValueError, match="環境変数GEMINI_API_KEYが設定されていません"):
    ...
    def test_translate_with_api_success(self, translator, mocker):
        ...
        expected_cmd = [
            ...
            '-H', 'x-goog-api-key: test-gemini-api-key',
            '-X', 'POST',
            '-d', '@/tmp/test.json',
    ...
    def test_translate_with_api_timeout(self, translator, mocker):
        ...
        mock_subprocess = mocker.patch('subprocess.run')
        mock_subprocess.side_effect = subprocess.TimeoutExpired(cmd=['curl'], timeout=30)
