...
class TestPlamoTranslator:
    ...
    def translator(self):
        ...
        with patch.object(PlamoTranslator, '_find_plamo_translate', return_value=('/usr/local/bin/plamo-translate', ['/usr/local/bin', '/usr/bin'])):
            return PlamoTranslator()

    def test_init(self, mocker):
        """Test PlamoTranslator initialization."""
        mock_find_plamo = mocker.patch.object(PlamoTranslator, '_find_plamo_translate', return_value=('/usr/local/bin/plamo-translate', ['/usr/local/bin', '/usr/bin']))
    ...
    def test_get_path_from_shell_fish(self, mocker):
        ...
        with patch.object(PlamoTranslator, '_find_plamo_translate', return_value=('/usr/local/bin/plamo-translate', [])):
            translator = PlamoTranslator()
        ...
        mock_ensure_common = mocker.patch.object(translator, '_ensure_common_paths', return_value=['/usr/local/bin', '/usr/bin', '/bin'])
        ...
        assert call_args == ['/opt/homebrew/bin/fish', '-l', '-i', '-c', 'printf "%s\\n" $PATH']
    ...
    def test_get_path_from_shell_timeout(self, mocker):
        ...
        mock_subprocess = mocker.patch('subprocess.run')
        mock_subprocess.side_effect = subprocess.TimeoutExpired(cmd=['bash'], timeout=10)
    ...
    def test_ensure_common_paths(self, mocker):
        ...
        mock_isdir.side_effect = lambda path: path in ['/usr/local/bin', os.path.expanduser('~/.local/bin')]
    ...
    def test_ensure_common_paths_already_present(self, mocker):
        ...
        input_paths = ['/usr/local/bin', '/usr/bin', '/bin', os.path.expanduser('~/.local/bin')]
        result = translator._ensure_common_paths(input_paths)
    ...
    def test_search_via_shell_found(self, mocker):
        ...
        mock_access.side_effect = lambda path, mode: path == '/usr/local/bin/plamo-translate'
    ...
    def test_show_dialog(self, translator, mocker):
        ...
        mock_subprocess.assert_called_once_with(['osascript', '-e', expected_script], check=False)

    def test_show_dialog_subprocess_error(self, translator, mocker, capsys):
        """Test dialog display when subprocess fails."""
        mock_subprocess = mocker.patch('subprocess.run', side_effect=subprocess.SubprocessError)
    ...
    def test_translate_with_plamo_cli_no_path(self, mocker):
        ...
        with patch.object(PlamoTranslator, '_find_plamo_translate', return_value=(None, [])):
            translator = PlamoTranslator()
            
            # Mock the _show_plamo_not_found_error method to prevent dialog display
            mock_show_error = mocker.patch.object(translator, '_show_plamo_not_found_error')
    ...
    def test_translate_with_plamo_cli_error(self, translator, mocker):
        ...
        with pytest.raises(ValueError, match="plamo-translateエラー: Translation failed"):
            translator.translate_with_plamo_cli("Hello")
    ...
    def test_translate_with_plamo_cli_timeout(self, translator, mocker):
        ...
        mock_subprocess = mocker.patch('subprocess.run')
        mock_subprocess.side_effect = subprocess.TimeoutExpired(cmd=['plamo-translate'], timeout=60)
    ...
    def test_run_empty_input(self, translator, mocker):
        ...
        mock_show_dialog.assert_called_once_with("テキストが選択されていません", "エラー")

    def test_run_plamo_not_found(self, mocker):
        """Test run method when plamo-translate is not found."""
        with patch.object(PlamoTranslator, '_find_plamo_translate', return_value=(None, ['/usr/bin', '/bin'])):
            translator = PlamoTranslator()
    ...
    def test_run_success(self, translator, mocker):
        ...
        mock_time = mocker.patch('time.time', return_value=1003.0)
        ...
        expected_message = "Translation result\n\nplamo-server時間: 1.50秒\n全体実行時間: 3.00秒"
    ...
    def test_run_unexpected_error(self, translator, mocker):
        ...
        assert result is False
        mock_dialog.assert_called_once_with("予期しないエラーが発生しました: Unexpected error", "エラー")
